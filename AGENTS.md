...
The left **Controls** dock is a single scrollable panel ([`py/qt_window_utils.py`](py/qt_window_utils.py) `build_scroll_dock_panel`): section info and progress at the top, **Tuning** (X/Y angle, AP, link angles), region/layout, parcellation, and warp options in the scroll body; **Previous / Next / Finish** are pinned at the bottom (always visible). On launch, the Napari window is clamped to the primary screen work area (`clamp_qt_window_to_available_screen`) so short/RDP displays do not clip controls. Optional **Flag section…** appends one JSON object per line to **`<align output / 01_slices>/.masonjar/alignment_flags.json`** (same directory tree as warped outputs / `Annotation_*.pkl`), e.g. `{ "sliceId", "filename", "index", "note", "timestamp" }`. This keeps notes out of `alignment.pkl` in the input folder.
