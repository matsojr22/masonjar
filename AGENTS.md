...
**Shared UI** ([`js/preprocess_wizard.js`](js/preprocess_wizard.js), [`js/max_datasets.js`](js/max_datasets.js)): signal branch, **source dataset** dropdown (max / sharpen / top-hat on that branch; **default max**), slice picker, **display min/max** sliders (0–255, client-side window on base image), pan/zoom on base only (CSS transform, no IPC), **Preview filter** button for ROI filter preview (optional **Auto-refresh after pan** checkbox, 5 s idle). Uses `_previews` PNG when available for faster pan/zoom. Tools are **not required** to run in sequence; users may chain (e.g. sharpen then top-hat) by choosing any existing dataset as source.
