...
The PyQt viewer uses a vertical **QSplitter**: collapsible **Parcellation tools** drawer (top) + side-by-side image views (bottom, stretch priority). **Background channel** is a **QComboBox** listing **`_previews/{sliceId}_*.png`** with fallback to **`00_dapi/{sliceId}.png`** as `DAPI (pipeline)` ([`py/adjust_channels.py`](py/adjust_channels.py)). **Paint brush — atlas region to draw:** dedicated **Search** box (`QCompleter`, `MatchContains`) above hierarchy tier/area combos; right-click on the atlas image still picks from the slice. Left-click brush uses the selected atlas id (empty background OK). **Refresh drawings** rebuilds the overlay from `current_label` without changing region IDs.
