...
**Interrupted / partial geometry apply:** [`js/geometry_state.js`](js/geometry_state.js) reads `.masonjar/geometry_apply_progress.json` and `geometry_apply_last_result.json` to block unsafe re-Apply. **Rebuild geometry** ([`pages/geometry_repair_wizard.html`](pages/geometry_repair_wizard.html), [`js/geometry_repair_wizard.js`](js/geometry_repair_wizard.js)) auto-runs a full-series audit (IPC `runGeometryFingerprintProbe` → [`py/geometry_fingerprint_probe.py`](py/geometry_fingerprint_probe.py); cross-channel **mask IoU** in [`py/geometry_orientation_match.py`](py/geometry_orientation_match.py), not intensity across channels), optional per-slice review, then `repair_mode: geometry` via [`py/apply_geometry.py`](py/apply_geometry.py). Orient: **Rebuild geometry**, **Finalize only** (reset JSON when disk done). Batch `apply_geometry` skips interrupted bundles.
