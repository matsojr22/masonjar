...
| `runAnnotationLabelAudit` | `runAnnotationLabelAudit` | `annotation_label_audit.py` | `annotationLabelAuditResult` JSON `{ ok, audit? }`. Args: annotation dir, structure_map path. |
...
**Menu** ([`js/menu_category.js`](js/menu_category.js)): **Semi-manual tissue edge cleanup** → [`pages/tissue_cleanup_wizard.html`](pages/tissue_cleanup_wizard.html) (preprocess category). **DAPI cleanup** and **Orient slices** live under the collapsed **Deprecated & Experimental** submenu (still reachable from workspace/CZI links). Canvas: static fit (no pan/zoom); **no mask overlay until you edit** (Attempt Auto, trace, or eraser); then green = keep, red = remove. Auto/guided masks pass through `ensure_keep_mask_polarity` in [`py/tissue_mask.py`](py/tissue_mask.py) so tissue stays green. Trace uses click-to-place points (`parse_stroke_points` accepts `{x,y}` or `[x,y]`); optional **Edge shrink** 0–5 px; eraser prunes orphan keep islands on mouseup.
...
**Shared UI** ([`js/preprocess_wizard.js`](js/preprocess_wizard.js), [`js/max_datasets.js`](js/max_datasets.js)): signal branch, **source dataset** dropdown (max / sharpen / top-hat on that branch; **default max**), slice picker, **display min/max** sliders (0–255), pan/zoom via CSS transform on `#preprocessPreviewTransform` (no IPC), **Preview filter** button (optional **Auto-refresh after pan** checkbox, 5 s idle). Initial navigation uses `_previews` PNG when available; after **Preview filter** the pane switches to the **native-resolution filter ROI** PNG from Python (same pixels as batch on that ROI) for parameter tuning — pan/zoom and preview again to refine. User must zoom in enough on large TIFFs before first preview (`PREVIEW_PIXEL_BUDGET` / min zoom gate). Evidence and display debugging history: [`docs/PREVIEW_FILTER_EVIDENCE.md`](docs/PREVIEW_FILTER_EVIDENCE.md). Tools are **not required** to run in sequence; users may chain (e.g. sharpen then top-hat) by choosing any existing dataset as source.
...
**Flow**: [`pages/intensity.html`](pages/intensity.html) (setup: paths, whole/hemisphere, DAPI, run mode) → **Configure outputs** → [`pages/intensity_wizard.html`](pages/intensity_wizard.html) + [`js/intensity_wizard.js`](js/intensity_wizard.js) (steps 2–4: CCF region picker, progress, summary). Setup is stashed in `sessionStorage` (`masonjar.intensity.setup`). **Process** writes run config and calls `runIntensity` with config path at IPC index 6. **Label resolution warnings:** [`js/annotation_label_audit.js`](js/annotation_label_audit.js) reads `<align_leaf>/.masonjar/annotation_label_audit.json` (refreshed on Adjust Save; IPC `runAnnotationLabelAudit` when stale). Setup + wizard show `#labelResolutionBanner` for stale annotations, mixed CCF levels, layer/coarse parcellation mismatch; wizard confirms before Process when issues present. Batch intensity preflight cells turn **amber** when audit cache reports issues.
...
The Align viewer uses a **top toolbar** on Napari’s main window (section info, progress, **Previous / Next / Finish**, **Flag section…**) plus two **left** docks: **Tuning** (X/Y angle, AP, link angles) and **Options** (region/layout, parcellation, tissue edge-cleanup warp). Napari **layer list** and **layer controls** are on the **right** (`relocate_napari_layer_docks_to_right`). On launch the window opens **maximized** with focus (`show_napari_maximized_and_activate` in [`py/qt_window_utils.py`](py/qt_window_utils.py)); Mason Jar and Napari docks/toolbars are forced visible (Napari 0.7 defaults custom chrome to hidden). **Run Align** minimizes Mason Jar and shows a handoff banner on [`pages/align.html`](pages/align.html); Mason Jar restores on Finish/Cancel/error (`handoffParentForExternalTool` / `restoreParentAfterExternalTool` in [`src/main.ts`](src/main.ts)). The legacy **Identify tissue damage** UI is removed; saved `damage_mask` values in old sessions are still honored at warp time only. Optional **Flag section…** appends one JSON object per line to **`<align output / 01_slices>/.masonjar/alignment_flags.json`** (same directory tree as warped outputs / `Annotation_*.pkl`), e.g. `{ "sliceId", "filename", "index", "note", "timestamp" }`. This keeps notes out of `alignment.pkl` in the input folder.

**Per-section tissue layout (whole vs left hemi):** Align setup defaults to **Automatic (recommended)** ([`pages/align.html`](pages/align.html), [`js/align.js`](js/align.js) IPC `-w auto|True|False`). [`py/align_tissue_layout.py`](py/align_tissue_layout.py) classifies each DAPI PNG (whole-brain vs left-hemisphere presentation); [`py/map.py`](py/map.py) always loads the full atlas volume and crops each atlas slice in `AtlasSlice.set_slice` from the per-section `hemisphere` (`W` or `L`). Napari **Section layout** combo overrides auto-detection per slice. Run manifest + `align_warp_report.json` store `layout_mode` and `slice_layouts`. Force-all-whole / force-all-hemi remain for legacy runs. **Isolate Regions** reads `slice_layouts` via [`py/align_layout_manifest.py`](py/align_layout_manifest.py) and applies per-slice `whole` masking when present.
...
[`py/count.py`](py/count.py) pairs **`Predictions_*.pkl`** with **`Annotation_*.pkl`** by **shared slice stem**. In project mode, the **Count** page selects active **predictions** and **slices** runs; output goes to `quantification/count/<slug>/`. [`js/pipeline_run.js`](js/pipeline_run.js) filters the count slice list to slices with a matching prediction PKL in the active predictions leaf and an annotation in the active slices leaf. **Count Brain** no longer exposes **Save layer info** — use parcellation (or Isolate Regions **Include cortical layers**) for layer-resolution outputs; Electron never passes `py/count.py --layers`. **Mixed-tier labels:** Count rolls up pixel atlas IDs at lookup time via [`resolve_count_label_id`](py/annotation_match.py) (parcellation context from `<slices leaf>/.masonjar/annotation_parcellation.json`; full-detail + layers off defaults to **areas** tier). Logs `LOG: count_rollup=…` once per run.
...
The PyQt viewer uses side-by-side image views as the central widget. A slim **header toolbar** holds section info, background **channel** combo, Previous/Next, **Toggle Overlay**, **Allow Adjustment**, and dock toggles. **Paint** and **Parcellation** each live in floatable **`QDockWidget`s** (left/right): Paint dock has region picker (**Search** + Hierarchy tier / CCF advanced Level + **Area** combo), paint-target strip (swatch, name, tier, adjustment state, brush size), **View** (opacity + zoom sliders), and brush/edit actions (Refresh drawings, Undo, Save); brush cursor ring when **Allow Adjustment** is on. **Search** and **Area** completers are **case-insensitive** and scoped to the **active content tier** (same list as the Area dropdown — semantic tier or CCF level in advanced mode; not the full catalog). Changing **Tier**, **Level**, or **CCFv3 depths** after painting shows a non-blocking mixed-resolution warning (status bar + one-time inform dialog); mixed maps remain allowed. An amber **paint dock** label and Save-time audit cache ([`py/annotation_label_audit.py`](py/annotation_label_audit.py) → `<align_leaf>/.masonjar/annotation_label_audit.json`) warn when labels mix CCF depths or mismatch parcellation. **Background channel** is a **QComboBox** listing **`_previews/{sliceId}_*.png`** with fallback to **`00_dapi/{sliceId}.png`** as `DAPI (pipeline)` ([`py/adjust_channels.py`](py/adjust_channels.py)). Overlay colors iterate **present label IDs** via [`resolve_label_color`](py/structure_catalog.py) (`structure_map` then catalog `color_hex_triplet`); stroke end and **Refresh drawings** / **Undo** call full `show_image_with_overlay()`. Right-click picks from the slice; left-click brush writes catalog atlas IDs into `current_label`.

**Parcellation (this section):** [`py/annotation_relabel.py`](py/annotation_relabel.py) rolls borders for **the current section only** (preview, apply with confirm from full-detail backup, restore fine). Bulk multi-section apply is **not** in Adjust — use **Parcellation (bulk)** wizard or batch. **Electron bulk wizard:** [`pages/parcellation_wizard.html`](pages/parcellation_wizard.html) + [`js/region_dual_list.js`](js/region_dual_list.js) **Included regions** dual-list (multi-select, Add/Remove/Add all/Remove all); config writes `included_region_ids` (legacy `excluded_region_ids` still honored in Python). **Batch step:** `parcellation` in batch wizard; headless [`py/apply_parcellation.py`](py/apply_parcellation.py) + [`py/annotation_exclusion.py`](py/annotation_exclusion.py) (`apply_inclusion`). Alignment **Finish** Napari parcellation unchanged.

### Window focus on launch ([`py/qt_window_utils.py`](py/qt_window_utils.py))

When the user starts **Alignment** from Mason Jar, the main process **`minimize()`s** the parent window after spawn and **restores** it when Align finishes (`handoffParentForExternalTool` / `restoreParentAfterExternalTool` in [`src/main.ts`](src/main.ts)). **Viewer/Editor** still uses **`blur()`** on spawn. The Python side calls [`show_napari_maximized_and_activate`](py/qt_window_utils.py) from `AlignmentController.start_viewer()` and [`raise_and_activate`](py/qt_window_utils.py) from `adjust.py` after `window.show()`. Each helper runs `show/raise_/activateWindow` immediately plus deferred `QTimer.singleShot` retries; Napari maximize also uses work-area geometry fit and Windows `ShowWindow(SW_MAXIMIZE)` on win32. No `WindowStaysOnTopHint`.
 ...
 **Windows LOG encoding:** `pythonShellEnv()` sets `PYTHONIOENCODING=utf-8` on win32; [`py/czi_common.py`](py/czi_common.py) reconfigures stdio to UTF-8 and `emit_log` / `emit_progress` use `_safe_print` so decorative Unicode in log strings cannot abort CZI jobs on cp1252 consoles.
  ...
  **CZI read path (aicspylibczi 3.x):** `read_image` returns `(ndarray, dims_list)` — never `np.asarray()` the raw tuple. [`py/czi_common.py`](py/czi_common.py) `read_czi_plane` unpacks the tuple, collapses fixed S/Z/C axes, and selects the **largest Y×X plane** when pyramid levels stack. Mosaic files try `read_mosaic(scale_factor=…, Z=…, C=…)` **without `S`** (libCZI rejects `S` on mosaic reads); multi-scene mosaics pass scene `region` from `get_mosaic_scene_bounding_box`. On mosaic read failure (including `PylibCZI_PixelTypeException`), fall back to `read_image`, then per-tile `read_image(M=…)` composite (`LOG: read_mosaic_tile_fallback`). **Sparse-Z counterstain:** [`z_indices_with_data`](py/czi_common.py) reads only Z positions with subblocks/tiles per channel (common when DAPI has one focal plane in a 9-deep stack); single-Z channels write a **2D TIFF** via existing `extract_z_stack` logic. DAPI previews use the brightest read plane, not blind mid-stack index. **Probe:** [`py/czi_probe.py`](py/czi_probe.py) uses `assess_mosaic_import(sample_read=False)` for bbox stitch heuristics plus `probe_channels_read` (per-channel `z_with_data` + sample read at a data-bearing Z). Wizard **Channel read check** box surfaces sparse-Z info and failed sample reads. **Extract** runs `assess_mosaic_import(sample_read=True, sample_scale=0.05)` once per `.czi` on first open. Reports `is_mosaic`, `m_tile_count`, `mosaic_stitch_status`, `likely_unstitched`, `mosaic_warnings`, `read_warnings`, and `channel_pixel_probe`. **Unstitched** Zen mosaics may import with seams or wrong geometry — users should stitch in ZEN first. **Single Z:** `extract_z_stack` writes a 2D TIFF (not a Z=1 stack); max projection copies/collapses a single plane instead of failing on `argmin` over equal dimensions.
...
**Orient previews (step 5) and standalone Orient:** Low-res previews are **uint8 PNG** at **5% linear** (`preview_scale` 0.05). **Orient display** reads `_previews` only: signal `_previews/{sliceId}_{branch}.png`, DAPI `_previews/{sliceId}_dapi.png`. **Pipeline DAPI** for Align/Adjust is `00_dapi/{sliceId}.png` only — **never TIFF** in `00_dapi`. CZI extract/repair **dual-writes** both DAPI PNG paths from the same plane. [`resolveOrientPreviewPath`](js/czi_import.js) (default channel `dapi`) and [`orientDapiPreviewPath`](js/czi_import.js) point at `_previews`, not `00_dapi`. [`listOrientDisplayChannels`](js/czi_import.js) labels DAPI as **DAPI (_previews)**. Legacy bundles: `ensureOrientDapiPreviewsFromPipeline` copies `00_dapi` PNG → `_previews/*_dapi.png`; **Repair previews** runs migrate + z-stack repair. Geometry is **per slice**, not per channel. Shared geometry UI: [`js/orient_geometry.js`](js/orient_geometry.js). Entry points: CZI wizard step 5 → [`py/apply_geometry.py`](py/apply_geometry.py); **Orient slices** ([`pages/orient.html`](pages/orient.html), [`js/orient.js`](js/orient.js)). **Check Orientation Consistency** ([`pages/geometry_repair_wizard.html`](pages/geometry_repair_wizard.html)) is linked from Orient only (not the preprocess menu).

**Re-import selected sections from CZI:** [`pages/czi_reimport_wizard.html`](pages/czi_reimport_wizard.html) + [`js/czi_reimport_wizard.js`](js/czi_reimport_wizard.js) — workspace hub, preprocess menu, or Orient (when blank DAPI detected). User picks sections + channels, confirms overwrite, then IPC `runCziImport` with `repair_mode: "reextract"` and `repair_targets` ([`js/czi_import.js`](js/czi_import.js) `buildReextractConfig`). Python [`py/czi_extract.py`](py/czi_extract.py) re-reads source `.czi` for each target (skips z-stack-first preview repair), overwrites z-stacks/previews, and **refreshes max TIFFs in the existing active run leaf** via `refresh_max_slices_in_run`. Blank preview audit: [`findBlankPreviews`](js/czi_import.js) / `BLANK_PREVIEW_MEAN_THRESHOLD` (default 8) flags nearly black orient previews; included in [`auditCziImportCompletion`](js/czi_import.js) as `blankPreviews`.
...
**Geometry ops (preview ↔ disk):** Per-slice pending edits are an append-only `geometry.ops` list (`rot90`, `flipX`, `flipY`) in [`js/orient_geometry.js`](js/orient_geometry.js). CZI wizard step 5 ([`js/czi_wizard.js`](js/czi_wizard.js)) and standalone **Orient slices** ([`js/orient.js`](js/orient.js)) share the same helpers: `applyGeometryAction` appends ops; preview uses `DOMMatrix` in click order (`geometryCssTransform` on `.czi-orient-tile-viewport`); geo clicks update one tile in-place (`updateTileGeometryDom`). Legacy `{ rotate, flipX, flipY }` in saved configs migrates on load via `legacyToOps` (same decomposition Python used: N×`rot90` for 90/180/270, then flips). [`py/apply_geometry.py`](py/apply_geometry.py) `compose_ops_from_spec` applies `spec.ops` literally when present; `rot90` → `np.rot90` **k=-1** (clockwise 90°). Copy-first-tile uses `cloneGeometry` (deep-copied `ops` per slice).
...
**Interrupted / partial geometry apply:** [`js/geometry_state.js`](js/geometry_state.js) reads `.masonjar/geometry_apply_progress.json` and `geometry_apply_last_result.json` to block unsafe re-Apply. **Check Orientation Consistency** / **Rebuild geometry** ([`pages/geometry_repair_wizard.html`](pages/geometry_repair_wizard.html), [`js/geometry_repair_wizard.js`](js/geometry_repair_wizard.js)) auto-runs a full-series audit (IPC `runGeometryFingerprintProbe` → [`py/geometry_fingerprint_probe.py`](py/geometry_fingerprint_probe.py); cross-channel **mask IoU** in [`py/geometry_orientation_match.py`](py/geometry_orientation_match.py), not intensity across channels), optional per-slice review, then `repair_mode: geometry` via [`py/apply_geometry.py`](py/apply_geometry.py). Primary entry: **Orient slices** → **Check Orientation Consistency** (also contextual links from CZI wizard step 5 when apply is blocked). Orient: **Rebuild geometry**, **Finalize only** (reset JSON when disk done). Batch `apply_geometry` skips interrupted bundles.
...
**Project open / run inventory:** [`openProject`](js/project.js) calls [`reconcileProjectRunsOnOpen`](js/pipeline_runs.js) after `migrateActiveRuns` and before `refreshProjectIndex`. Discovery scans every output role on disk (depth **3** for max/sharpen/tophat); invalid stored `active_runs` slugs are cleared; a role with exactly one discovered run gets that run set automatically. Multiple runs stay unselected until the user picks from **Completed tasks**. `.masonjar/runs_catalog.json` is a diagnostic inventory written on rescan. Partial align warp failures stay in **Alignment issues** (`step_failures.align`), not active-run reconcile.

**Import handoff UX:** CZI import writes max runs under `03_max/{branch}/max/<slug>/` (discovered at depth **3** in [`js/pipeline_runs.js`](js/pipeline_runs.js)). After finish, [`js/import_handoff.js`](js/import_handoff.js) drives step 6 **What's next** ([`pages/czi_wizard.html`](pages/czi_wizard.html)), the workspace **Next step: atlas alignment** banner (`#importNextStepsBanner` on [`pages/workspace_menu.html`](pages/workspace_menu.html)), and an info alert on [`pages/max.html`](pages/max.html). **Completed tasks** ([`js/project_files.js`](js/project_files.js)) lists the active max run plus read-only DAPI/preview rows from import. Primary next step is **Align Sections**; if alignment is difficult, users are pointed to counterstain cleanup under Image preprocessing (DAPI cleanup, tissue edge cleanup, Orient).
